...
**最后核对：**2026-07-26（分支`v2`，commit `feedb06` + 本次文档一致性修订）

**注意：**D5与D6偏离的是**权威链更靠前的文档**（D5偏离第1位的review-adjustments，D6偏离第3位的Design Spec v3），不只是实现细节。它们由用户明确决定，但按CLAUDE.md「冲突时靠前者优先」的规则，读到相关章节时必须以本文件为准。
...
**状态：已定不做**（2026-07-26，用户决定）。

| | |
|---|---|
| **spec原文** | `2026-07-19-remote-claude-workspace-v2-review-adjustments.md`§2.6（**权威链第1位**）要求「增加文件系统硬配额的实施任务」；Design Spec v3 §195把技术选型**固定为**每项目loop设备文件系统并写明「不得留待实现时再定」，验收26要求真实容器验证 |
| **实际决定** | 不实施。`deploy/quota-setup.sh`保留在仓库但**不接入任何流程**，文件头已标注勿执行 |

**这条偏离的是权威链最靠前的文档，不是实现细节，因此必须显式记录。**

**技术事实：**脚本创建的卷名是`<slug>-workspace`，compose实际使用的卷带项目前缀（`docker compose config`实测为`deploy_project-a-workspace`）——两者不是同一个卷。脚本会正常退出并打印`capped at NN GiB`，容器却仍挂着不受约束的普通命名卷，**没有任何报错**。要让它生效必须改卷布局，用户2026-07-26决定沿用现状的普通命名卷（`plans/2026-07-26-compose-render-plan.md`§4.4）。

**承担的后果：**逻辑配额只统计走同步接口的文件，容器内直接写盘可突破配额并撑爆宿主机磁盘，同节点全部项目一并受影响。**对外只能称"配额"，不得表述为"保证不超过"。**

**替代防线**（`specs/2026-07-25-ccw-console-fleet-design.md`§12.1的N4，尚未实施）：Docker`data-root`独立分区 + Postgres数据移出data-root + 磁盘水位告警。它们只把后果从"整台机器死亡且救不回来"降到"一起降级、机器可救"，**不消除项目间的互相影响**。

**完整取舍记录见`docs/STATUS.md`的T1。若将来重开，回到渲染计划§4.2。**

---

## D6. 使用形态从「本人双项目」改为「一节点多项目、可分配给他人」

**状态：已定**（2026-07-26，用户确认）。

| | |
|---|---|
| **spec原文** | Design Spec v3 §32合规立场：「本系统**仅供同一账号所有者本人的两个项目使用**……**不向第三方转售访问权**」；§26开发范围：「双项目个人版」 |
| **实际形态** | 一台节点授权一个Claude账号，节点上**最多3个项目**共用该账号的上游额度；项目可以全归管理员自己，**也可以分配给他人使用**（`specs/2026-07-25-ccw-console-fleet-design.md`§7.2） |

**为什么必须记录：**旧表述不是措辞陈旧，而是与现形态**直接矛盾**——README曾据此写有同样的合规立场段落，已于本次修订删除并改为如实描述（README「使用形态与边界」）。留着会造成对外文本与实际产品不符。

**本仓库不作断言的部分：**多人共用同一个Claude账号在上游服务条款下是否被允许，**没有依据可下结论，由部署者自行核实**。文档只如实描述系统行为，不提供合规背书。

**连带影响：**

1. **额度闸门从「锦上添花」变成「必需品」**——多个使用者共用一个上游池时，内部闸门是唯一能阻止其中一个吃光全机额度的机制。这是Console设计§12.3把P0-1定为**上线阻断项**的原因。**2026-07-26已接线**（N1+N2），但只有单测证据、未真机验证，且加权系数处于"先记账、后校准"第一阶段——闸门当前实际不会拦人
2. **隔离性残留缺陷的性质变了**：共享Claude HOME导致命令历史与shell快照互相可见（设计§7.5）、容器逃逸影响全机（`worker-agent`持有docker.sock）。自用时这些是可接受的；分配给他人时，设计§7.5要求**写入服务条款**——**该服务条款目前不存在**
3. Design Spec v3 §32与§26的文本**未修订**，以本条目为准
