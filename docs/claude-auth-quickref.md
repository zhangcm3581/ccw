...
> **首选：直接在后台做。**节点详情页上：
>
> - 「节点诊断」卡片 → 运行检查：容器状态、每个项目的登录状态、凭据文件属主、磁盘水位，
>   一次跑完（等价于本文第 1、3 节与「登录后仍是 false」那节的全部命令）
> - 「授权 Claude 账号」卡片 → 开始授权：起会话、显示终端画面、粘贴授权码
> - 同卡片的「重建 <slug>」：验证凭据随卷持久（等价于 `--force-recreate`）
>
> **全程不用登机。**本文用于后台不可达、或后台报错后要手动深挖时。
