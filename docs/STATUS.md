...
- **额度档位与池上限自动校准**（2026-08-02）：项目限额不再是写死的绝对值，
  ...
  **模板引用不存在的字段会把整页渲染截断**（2026-08-02 真机）：`.TierEffective`
  漏加进 `provision.NodeProjectUsage`，而 Go 模板引用缺失字段**只在运行时报错**
  ——构建通过、既有测试全绿（没有一条测试用 `Tier` 非空的行渲染过页面，
  而那段在 `{{if .Tier}}` 之内）。出错时**已写出的部分照样送给浏览器**，
  后面直接截断：表现是"项目只剩一个、档位下拉不见了"，看着像功能坏了。
  `admin_usage_render_test.go` 用带档位的真实数据整页渲染并断言关键片段，
  就是当时缺的那条。
  **用量页报的是实际生效的限额**（2026-08-02 真机暴露）：档位是闸门在运行时
  用 `quota.ApplyTier` 算的，**从不写回 `projects.five_hour_limit` 那一列**，
  而页面原来直接报那一列——于是档位已经生效、页面却一直显示那个没被用到的
  绝对值，用户据此判断"指派没成功"。现在报告里先套一遍 ApplyTier 再输出，
  并带一个 `tier_effective` 标记（判断靠"限额变没变"，而不是把 ApplyTier 的
  条件再抄一遍——抄一遍就会有两套规则）。挂了档位但容量未校准时页面明说原因。
  用量页挂在**节点下面**（`/admin/nodes/{id}/usage`，节点详情页右上角进入）：
  项目、档位表（`quota_tiers`）、Claude 账号全都是节点本地的东西，平铺成一页
  会让"这个设置对哪台生效"变得含糊——review 里那两个 bug（档位指派发到错节点、
  档位表看着像全机队设置）正是从平铺结构来的。按节点分开之后页面上只有一台，
  指不错。旧的 `/admin/usage` 重定向到机队列表。
  该页上可改档位百分比、给项目指派档位（CSRF + 审计），
  ...
  最后一次会话的时刻。
  **code review 修了两处**（2026-08-02）：① 全新安装时 `pool_*_limit` 默认是
  `MaxInt64`（迁移 002 的"无限制"哨兵），`MaxInt64 × 3300 / 10000` 回绕后得 **0**
  ——挂上档位的项目限额变成 0，**立刻永久受限**，而那正是校准还没跑起来时的
  默认状态。现在池未校准/为 0/会溢出时一律沿用绝对限额，且任一窗口算不出来
  就整个不套用（只换一半会得到自相矛盾的限额）。② 档位指派原来用全页共用的
  一个 node，多节点时会把项目挂到另一台机器上——那台要么没这个 slug、
  要么恰好有同名项目（改错人还不报错）。现在每行各带自己的节点 ID；
  档位表本身也是节点本地的，页面上已标出是哪一台。
  **连接时立刻写一次本项目额度**（2026-08-02 真机）：那个文件原本只由 30 秒的
  额度循环写，于是新会话头 30 秒里状态行读不到本项目额度，退回显示账号级
  （标签「账号5h」），过一会儿又自己变成本项目——看着像界面在抽风。
  容器一重建 `/tmp` 就是空的，所以**每次新连接都会经过这个窗口**。
  现在 startPTY 在附着之前先算一次并写入，窗口消失。
