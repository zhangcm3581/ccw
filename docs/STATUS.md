...
- **同步目录与项目选择器**（2026-07-31）：此前同步的是"你碰巧所在的那个目录"，
  ...
  **盘符根、家目录、同步目录自身不提示「登记当前目录」**——在那些地方回车一下
  ...
  不会把人困住。
- **窗口对齐 Claude 的重置点**（2026-08-02）：此前项目额度用**滚动窗口**
  （`now-5h`/`now-7d`），而 Claude 账号是**固定窗口**——到 `resets_at` 归零。
  两种模型不一致带来两个问题：① Claude 重置了，项目额度还背着重置前的用量，
  要等它慢慢滑出去；② 校准拿"我们的滚动累计 ÷ Claude 的固定窗口百分比"，
  两边量的不是同一段时间，跨过重置点时比值失真。
  现在把状态行回传的 `resets_at` 存进 `accounts`（迁移 005），窗口起点 =
  `resets_at - 窗口长度`，闸门与校准用同一个边界。
  **已经过去的 `resets_at` 仍然有用**：它说明重置已经发生，那之前的用量属于
  上一个窗口。所以窗口起点取 `max(resets_at, now-窗口长度)`——专治
  "项目被断开之后没人回传新快照"：被断的项目没有会话、拿不到新的 resets_at，
  但旧值已经足够让它在重置那一刻恢复，而不是等自己那 5 小时慢慢滑完。
  上界那一半同样必要：快照太旧时退回滚动，否则会把好几天的用量塞进
  一个"5 小时"窗口。
  **拿不到边界（从没有过快照）时退回滚动窗口**——那是改动前的行为，
  安全的降级；过期的边界尤其不能用，它算出的起点在更早的过去，
  会把本该滑出的用量重新算进来。
...
### T3 管理后台不再要求两步验证（2026-08-02 按要求移除）

登录只校验用户名与密码。剩下的防护是：登录限速（IP + 用户名两个维度）、
统一错误、服务端可撤销会话、CSRF，以及 IP 白名单。

**白名单为空时等于不限制**（`ipAllowed` 的空列表分支直接放行），
也就是说没配 `CCW_ADMIN_ALLOWLIST` 时，公网后台只剩一道密码——
而后台权限等同全机队 root。`create-admin` 的输出里已加了这条提醒。

TOTP 的存储字段与生成逻辑保留未删（列是 NOT NULL；留着让"想再开回来"
只是恢复 auth.go 里几行校验），只是登录不再校验、创建账号也不再打印密钥。
