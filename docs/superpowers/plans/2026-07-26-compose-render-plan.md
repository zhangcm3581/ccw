...
**状态：****R1–R7已实施**（2026-07-26同日）。R8（磁盘水位告警）的数据源并入C12的`ccwadmin status --json`，告警本体归Console巡检（设计§11.5），本计划不再跟踪。
**验证情况：**B1–B8由单测与本机`docker compose config`验证通过（B1实测：渲染的双项目输出与原手写文件经config归一化后逐字节相同）；**B9（真实节点加第3个项目）未跑，不得记为完成**；B10的静态部分（卷名与main布局一致）已由黄金文件锁定。
**实施偏差：**①黄金文件只有1/2/3项目——§7的`compose-10.golden.yaml`与B2的"10个项目"写于§6项目数上限（3个）定案之前，10个项目现在会被正确拒绝，该文字不再适用；②slug总长按§6文字取2–32（§6正则允许单字符，取更严格口径）。
**日期：**2026-07-26
**分支：**`v2`
