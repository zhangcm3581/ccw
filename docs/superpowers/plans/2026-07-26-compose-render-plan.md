...
**代价必须记录在案：**硬配额（P1-5）**不会**因本计划而修复，`quota-setup.sh`仍处于「执行了也不生效」的状态。后果是**客户在容器内直接写盘可撑爆宿主机磁盘，拖垮同机全部客户**——逻辑配额（`internal/storage`）只统计走同步接口的文件，拦不住容器内直接写入。
...
| 中 | Docker `data-root`独立分区/盘 | 撑爆的是该盘，宿主机OS与Postgres存活，可SSH救援 | bootstrap的`install-docker`步骤 |
| 高 | loop-mount硬配额（方案B/C） | 单客户写满只影响自己 | 暂不做，保留在P1-5 |
...
**前置实测已取消。**原计划的V1（bind传播行为）随方案C不做而失去意义；V2（`quota-setup.sh`卷名不兼容）不再是本计划的闸门——该结论已记入`docs/STATUS.md`的P1-5，留待将来真要上硬配额时验证。
