...
`usage.Weights{Input, Output, CacheRead, CacheWrite}`把token数折算成"内部额度单位"。**这些数字目前没有任何依据**——生产无配置入口，测试里的值是随便取的。

**本计划提议的默认值**（比例参照公开的相对定价量级，**是估算不是官方口径**）：

| 字段 | 默认权重 |
|---|---|
| `Input` | 1 |
| `Output` | 5 |
| `CacheRead` | 1（十分之一量级，取整后为1） |
| `CacheWrite` | 1 |

配置入口：`CCW_USAGE_WEIGHTS="1,5,1,1"`，`config.Load`按CLAUDE.md规则**缺失即硬失败**，不设默认值。

**这个取值需要你确认**（§8开放问题1）。它直接决定`five_hour_limit`（`ccwadmin`当前默认1_000_000）意味着多少实际用量——权重不定，配额数字就是无意义的。
...
| U7 | 采集goroutine的panic不影响执行goroutine，反之亦然 | 一个失败拖垮闸门 |
...
1. **加权系数取值**（§3.1）——需要你定。它决定`five_hour_limit`的数值含义；权重不定，配额就是无意义的数字。建议先按`1/5/1/1`跑，观察一周实际分布后校准。
2. **池上限的初始值**——`accounts`加列后填什么？取决于你的订阅档位与愿意承受的超卖倍率（设计§15开放问题2仍未关闭）。
3. **首次采集的历史积压**：已运行一段时间的部署，首轮`Scan`会把全部历史JSONL入账，可能瞬间把项目推到"已超额"。是否需要一个"只采集从接线时刻起的新事件"的开关？**倾向不做**——历史用量本来就消耗了真实额度，如实计入更诚实；但要在UPDATE.md里提示这个现象，避免被当成bug。
4. 每事件一次INSERT，首轮积压可能有数千次往返。是否需要批量写入？**倾向先不做**——批量会让"Sink失败游标不前进"的语义变复杂（§3.4），等实测确认是瓶颈再优化。
