...
**每个本地目录是一个独立的工作区**：客户端按当前目录的绝对路径算出工作区键，
云端按键分开存放。在 `~/code` 与 `~/work` 下各跑一次，两边的文件互不可见；
云端目录是 `<workspace-root>/<slug>/<工作区键>/`，终端的 tmux 会话也落在同一个目录里，
会话名就是工作区键（因此不同目录各有一个互不打扰的终端，断线重连回到自己那个）。
