...
func TestSyncModeRWWhenUnderQuota(t *testing.T) {
	...
	if got := syncModeFor(context.Background(), q, svc, "pid", time.Now(), func(string, ...any) {}); got != "rw" {
...
func TestSyncModeCleanupWhenProjectOverQuota(t *testing.T) {
	...
	if got := syncModeFor(context.Background(), q, svc, "pid", time.Now(), func(string, ...any) {}); got != "cleanup" {
...
func TestSyncModeCleanupWhenAccountPoolExhausted(t *testing.T) {
	...
	d, err := checkProject(context.Background(), q, svc, "pid", time.Now())
	if err != nil {
...
func TestSyncModeFailsClosed(t *testing.T) {
	for name, q := range map[string]fakeQuotaLookup{
		...
		logged := false
		got := syncModeFor(context.Background(), q, svc, "pid", time.Now(),
...
// limitsFor必须把账号池上限真的读进来——写死极大值等于池闸门不存在。
func TestLimitsForUsesAccountPoolLimits(t *testing.T) {
	q := fakeQuotaLookup{p: testProject(), pool5: 4242, pool7: 424242}
	lim, err := limitsFor(context.Background(), q, testProject())
	if err != nil {
		t.Fatalf("unexpected: %v", err)
	}
