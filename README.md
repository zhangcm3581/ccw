...
./cclaude --api https://你的域名                # 首次指定API地址（会自动补 /api 前缀并记住）
./cclaude                                      # 之后直接运行；首次会提示输入CDK（不回显）
```
...
- **额度闸门未经真机验证**：用量采集已于2026-07-26接线（`usage_events`会被写入，超额会关终端并把同步降级为cleanup），但**全部只有单测证据**。且加权系数目前是估算起点、限额刻意设得很宽——处于"先记账、后校准"阶段，实际不会拦人。在校准完成前，不要依赖它防止某个项目吃光全机额度。
